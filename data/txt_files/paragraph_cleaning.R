...
# doc1 <- pdf_ocr_text(files[1])
  #doc1 <- str_split(doc01, pattern = "")
  #cleaning document
  doc01 <- gsub(pattern="\\W" , replace = " ", doc01)%>% gsub(patter = "\\d", replacement = " ", doc01)
  doc01<- removeWords(doc01, c(stopwords("english"),"will","can","on","table","da","other","only", "the", "other", "others", "The"))
  doc01 <- gsub(pattern = "\\b[A-z]\\b{1-3}", replace= " ", doc01)
  doc01 <- stripWhitespace(doc01)
  doc01 <- stringi::stri_remove_empty(doc01)
  cleaned <- cleaned %>% append(doc01)
  names <- names %>% append(rep(file, length(doc01)))
}
#doc1 <- stringi::stri_remove_empty(doc01)
